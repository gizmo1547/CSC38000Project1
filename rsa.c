...
#define NEWZ(x) mpz_t x; mpz_init_set_ui(x,0)
#define NEWRANDST(x) gmp_randstate_t x; gmp_randinit_mt(x);
...
int rsa_keyGen(size_t keyBits, RSA_KEY* K)
{
	...
	size_t bytes = keyBits / 8;
	unsigned char * entropy = calloc(bytes, 1);

	FILE* frand = fopen("/dev/urandom", "rb");
	fread(entropy, 1, bytes, frand);
	fclose(frand);
	...
	NEWZ(seed);
	BYTES2Z(seed, entropy, bytes);
	free(entropy);

	NEWRANDST(rstate);
	gmp_randseed(rstate, seed);

	mpz_urandomb(p, rstate, keyBits);
    ...
    do
    {
        mpz_urandomm(e, rstate, fi);
        mpz_gcd(gcd, e, fi);

    }  while(mpz_cmp(e, fi) >=0 || mpz_cmp_ui(gcd, 1) != 0);
	...
	mpz_clears(seed, p, q, e, d, n, fi, gcd, NULL);
...
size_t rsa_encrypt(unsigned char* outBuf, unsigned char* inBuf, size_t len, RSA_KEY* K)
{
	...
	if(inBuf == NULL)
	{
		puts("NULL input buffer. Encryption stopped.");
		return bytes_written;
	}

	if(outBuf == NULL)
	{
		puts("NULL output buffer. Encryption stopped.");
		return bytes_written;
	}

	NEWZ(mess);
	NEWZ(ctxt);
	BYTES2Z(mess, inBuf, len);

	mpz_powm(ctxt, mess, K->e, K->n);
	Z2BYTES(outBuf, bytes_written, ctxt);

	mpz_clears(mess, ctxt, NULL);
...
size_t rsa_decrypt(unsigned char* outBuf, unsigned char* inBuf, size_t len, RSA_KEY* K)
{
	...
	NEWZ(mess);
	NEWZ(ctxt);
	BYTES2Z(ctxt, inBuf, len);

	mpz_powm(mess, ctxt, K->d, K->n);
 	Z2BYTES(outBuf, bytes_written, mess);
